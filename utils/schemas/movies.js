...
const movieTitleSchema = joi.string().max(80).trim();
const movieYeartSchema = joi.number().min(1888).max(2077);
const movieCoverSchema = joi.string().uri().trim();
const movieDescriptionSchema = joi.string().max(300).trim();
const movieDurationSchema = joi.number().min(1).max(300);
const movieContentRatingSchema = joi.string().max(5);
const movieSourceSchema = joi.string().uri().trim();
const movieTagsSchema = joi.array().items(joi.string().max(50).trim())
